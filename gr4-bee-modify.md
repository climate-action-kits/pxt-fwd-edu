...
We are going to use something called ``||Variables:Variables||`` to track this number. In this program, our variable is named 'bugVisits'.
...
## Challenge Step 5 
Did you place it correctly? The ``||fwdSensors:set all ledRing LEDs to||`` block should be placed within the ``||basic:forever||`` block, above the ``||logic:if true then||``. <br> Use the lightbulb icon to check your code!
